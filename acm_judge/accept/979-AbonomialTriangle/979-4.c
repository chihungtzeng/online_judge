...
int compareSegment(const void *a, const void *b){
	...
	x = (struct segment *)a;
	y = (struct segment *)b;
	if (x->idx1 != y->idx1) return x->idx1 - y->idx1;
	else  return  (x->idx2 - y->idx2); 
}

struct segment *newSegment(int i, int j){
	struct segment *s;
...
int main(){
	...
	while(2 == scanf("%hd %hd", &x, &y)){
		...
		while((!done) && (nCandidate > 0)){
			...
			if(VERBOSE){
				printf("test point i=%hd and j=%hd\n",i, j);
			}			
			...
			for(k = 0; (k<nPoint) && (!done); k++){
				...
				target[0] = newSegment(min(i, k), max(i,k));				
				...
				target[1] = newSegment(min(j, k), max(j,k));
