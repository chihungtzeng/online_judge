...
int checkValid(char cookie[MAX][MAX], short *rpos, short *cpos){
	short r, c, d, i;
...
int findSolution(char cookie[MAX][MAX]){
	...
	if(checkValid(cookie, &r, &c) == VALID){
		if(DEBUG){
	...
	if((r<0) || (r >=R) || (c<0) || (c>=C)){
		if(VERBOSE){
			printf("wrong position for unknown cell: r=%hd c=%hd", r, c);
		}
		return 0;
	} 	

	if(VERBOSE){
		printf("try-and-error on cell r=%hd c=%hd\n",r,c);
	}
...
int main(){
	...
	while(2 == fscanf(fp, "%hd %hd", &R, &C)){
		...
		for(i=1; i<R+C-2; i++){
			...
			else if(diag[i] == nDiagPos[i]){
				for(j=0; j<nDiagPos[i]; j++){
					cookie[diagPos[i][j].row][diagPos[i][j].col] = COOKIE;
				}
				diagFlag[i] = DONE;
			}
		}
