...
void computCostFunction(){
	...
	for(i=0; i<nCity; i++){
		for(j=0; j<nInitMaternity; j++){
			...
			dist = sqrt((city[i].x - city[dest].x)*(city[i].x - city[dest].x) + 
					(city[i].y - city[dest].y)*(city[i].y - city[dest].y));
...
*/
	...
	memcpy(closeListBak, closeList, sizeof(short)*closeListIndex);
	...
	if (curCost + extraCost > bestAns) return INTMAX;
...
int main(){
	...
	outfp = (VERBOSE)? fopen("shit.txt", "w") : stdout;
	...
	while(1 == fscanf(fp, "%hd", &nCity)){
		...
		for(i=0; i<nCity; i++){
			fscanf(fp, "%s %d %d %d", cityName, &np, &x, &y);
