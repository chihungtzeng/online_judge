...
int getSmallFactor(long long n){
	...
	for( ; primeIndex<nPrime; primeIndex++){
		if(n % prime[primeIndex] == 0) return prime[primeIndex];
	}
	return 0;
}
...
long long factoring(long long input){
	long long factor;
	int step, i, k;
	long long result = 1;
	primeIndex = 0;
	while(0 != (factor = getSmallFactor(input))){
		/*find a small factor*/
		#if DEBUG
			printf("factor: %lld\n", factor);
		#endif
	...
	while(isProbablePrime(input) == COMPOSITE){
		step = 1;
		do{
			factor = getBigFactor(input, step);
			step++;
		} while(factor == 1);
