...
class Main{
   ...
   public static long pMinusOne(BigInteger N) {
        BigInteger divisor;
        BigInteger target, base;
        int count=2;
        
        
        target = new BigInteger("4");
        
        do{        	
        	target = target.modPow(new BigInteger(""+count), N);
        	count++;
        	divisor = N.gcd(target.subtract(BigInteger.ONE));
