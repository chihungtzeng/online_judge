...
int ll2BigInteger(long long x, struct BigInteger *b){
	short i;
	b->nEle = 0;
	while((x > 0) && (b->nEle < NELE)){
		b->ele[b->nEle] = x % BASE;
		x = x / BASE;
	}
	if(x == 0){
...
int bigIntegerSquare(struct BigInteger *result, struct BigInteger *b){
	assert( (result != NULL) && (b != NULL));
	short i;
	long long carry = 0;
...
int printBigInteger(struct BigInteger *b){
	short i;
	printf("%d", b->ele[nEle - 1]);
	for(i=nEle-2; i>=0; i--){
		printf("%08d", b->ele[i]);
	}
	return EXIT_SUCCESS;
...
int nextValue(long long x, struct BigInteger *b){
	struct BigInteger operand;
	if(EXIT_FAILURE == ll2BigInteger(x, &operand)){
		printf("conver %lld to big integer failed\n",x);
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
