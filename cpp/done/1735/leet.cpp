...
class Solution {
 public:
  vector<int> waysToFillArray(vector<vector<int>>& queries) {
    ...
    vector<vector<int>> combs(20001, vector<int>(20001));
    for (auto& q : queries) {
  ...
  int calc(const int ndigit, const int prod, vector<vector<int>>& combs,
           vector<int>& primes) {
    if (ndigit == 1 || prod == 1) {
      return 1;
    }
  ...
  int calc_comb(int n, int p, vector<vector<int>>& combs) {
    if (n == p || p == 0) return 1;
    if (n == p + 1 || p == 1) return n;
    if (n < p) {
      return 0;
    }
    if (combs[n][p] > 0) return combs[n][p];
    int res =
