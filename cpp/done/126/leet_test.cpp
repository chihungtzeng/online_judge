...
TEST(kerker, t0) {
  ...
  EXPECT_EQ(sol.findLadders(beginWord, endWord, wordList).size(), 0);
}
...
TEST(kerker, t1) {
  ...
  auto actual = sol.findLadders(beginWord, endWord, wordList);
...
TEST(kerker, t2) {
  ...
  std::vector<std::vector<std::string>> expect{{"a", "c"}};
  auto actual = sol.findLadders(beginWord, endWord, wordList);
  EXPECT_EQ(expect, actual);
}
...
TEST(kerker, t4) {
  Solution sol;
  std::string beginWord = "nanny";
  std::string endWord = "aloud";
  ...
  std::vector<std::vector<std::string>> expect{
      {"nanny", "canny", "candy", "sandy", "sands", "sends", "seeds",
       "sleds", "slews", "clews", "clefs", "cleft", "cleat", "bleat",
       "bloat", "float", "flout", "clout", "cloud", "aloud"},
      {"nanny", "danny", "dandy", "sandy", "sands", "sends", "seeds",
       "sleds", "slews", "clews", "clefs", "cleft", "cleat", "bleat",
       "bloat", "float", "flout", "clout", "cloud", "aloud"}};
  auto actual = sol.findLadders(beginWord, endWord, wordList);
  EXPECT_EQ(expect, actual);
...
TEST(kerker, w1) {
  Word w1(0, "nanny");
  LOG(INFO) << w1.masked_nbrs_;
  EXPECT_EQ(w1.masked_nbrs_.size(), 5);
}
#if 0
#endif
