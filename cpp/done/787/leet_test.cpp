...
TEST(kk, t1) {
  ...
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 1), 200);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 0), 500);
}

TEST(kk, t2) {
  Solution sol;
  int n = 6;
  vector<vector<int>> edges = {{0, 1, 200}, {0, 2, 64}, {0, 5, 1},
                               {5, 2, 32},  {5, 4, 2},  {4, 2, 16},
                               {4, 3, 4},   {3, 2, 8},  {2, 1, 128}};
  int src = 0, dst = 1;

  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 0), 200);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 1), 192);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 2), 161);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 3), 147);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 4), 143);
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, 5), 143);
}

TEST(kk, t3) {
  Solution sol;
  int n = 5;
  vector<vector<int>> edges = {{4, 1, 1},  {1, 2, 3}, {0, 3, 2},
                               {0, 4, 10}, {3, 1, 1}, {1, 4, 3}};
  int src = 2, dst = 1, k = 1;
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, k), -1);
}
...
TEST(kk, t4) {
  ...
  int src = 0, dst = 4, k = 2;
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, k), 7);
}
...
TEST(kk, t5) {
  ...
  EXPECT_EQ(sol.findCheapestPrice(n, edges, src, dst, k), 47);
}
